...
void UMultiplayerSessionSubsystem::CreateSession(int32 NumPublicConnection, FString MatchType)
{
	...
	{
		if (GEngine)
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Green, FString(TEXT("SessionInterface invalid")));
		return;
	}
	...
	if (ExistingSession != nullptr)
	{
		...
		LastNumPulbicConnections = NumPublicConnection;
	...
	if (!SessionInterface.IsValid())
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Red,
				FString(TEXT("FindSession failed because SessionInterface is invalid"))
			);
		}
		return;
	}
	...
	if (!SessionInterface->FindSessions(*LocalPlayer->GetPreferredUniqueNetId(), LastSessionSearch.ToSharedRef()))
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Red,
				FString(TEXT("FindSessions Failed"))
			);
		}
...
void UMultiplayerSessionSubsystem::OnFindSessionComplete(bool bWasSuccessful)
{
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Blue,
			FString(TEXT("OnFindSessionComplete"))
		);
	}
	if (SessionInterface)
	{
	...
	if (LastSessionSearch->SearchResults.Num() <= 0)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Red,
			FString(TEXT("Don't find any session"))
		);
		MultiplayerOnFindSessionsComplete.Broadcast(TArray<FOnlineSessionSearchResult>(), false);
		return;
	}
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Blue,
			FString::Printf(TEXT("MultiplayerOnFindSessionsComplete boardcast %d"), LastSessionSearch->SearchResults.Num())
		);
	}
...
void UMultiplayerSessionSubsystem::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (bWasSuccessful && bCreateSessionOnDestroy)
	{
		...
		CreateSession(LastNumPulbicConnections, LastMatchType);
	}
