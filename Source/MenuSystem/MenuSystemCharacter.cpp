...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	if (!OnlineSessionInterface.IsValid())
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Blue,
				FString(TEXT("OnlineSessionInterface is not valid"))
			);
		}
		return;
	}
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Blue,
			FString(TEXT("OnlineSessionInterface->CreateSession"))
		);
	}
...
void AMenuSystemCharacter::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Green,
			FString::Printf(TEXT("Find session result %s"), bWasSuccessful ? "true" : "false")
		);
	}
...
void AMenuSystemCharacter::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Resut)
{
	if (!OnlineSessionInterface.IsValid()) return;
